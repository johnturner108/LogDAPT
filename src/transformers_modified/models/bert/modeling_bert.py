...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        ## pad_token_id是用于pad的token的id，为0
        ## 每一个vocab映射为一个hidden_size大小的向量
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
        ## 一共512个位置，每个位置映射为一个hidden_size大小的向量
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        ## 一共两种token type，分别映射为hidden_size大小的向量
        ...
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps) ## config.layer_norm_eps = 1e-12
        self.dropout = nn.Dropout(config.hidden_dropout_prob) ##　During training, randomly zeroes some of the elements of the input tensor with probability p using samples from a Bernoulli distribution.
...
class BertMLMSBOHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, max_pair_targets):
        super().__init__()
        self.predictions = BertLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
        self.pair_target_predictions = BertPairTargetPredictionHead(config, bert_model_embedding_weights, max_pair_targets)

    def forward(self, sequence_output, pooled_output, pairs):
        prediction_scores = self.predictions(sequence_output)
        # seq_relationship_score = self.seq_relationship(pooled_output)
...
class BertPairTargetPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, max_targets=15, position_embedding_size=200):
    ...
    def forward(self, hidden_states, pairs):
        bs, num_pairs, _ = pairs.size()  ##  _ 要是获取到的话那就是2，pair的size为(batch_size, num_pairs, 2)
...
@dataclass
class BertForMLMSBOOutput(ModelOutput):
    """
    Output type of [`BertForPreTraining`].
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        r"""
            ...
            next_senltence_label (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
...
class BertForMLMSBO(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        # self.cls = BertPreTrainingHeads(config)
        max_pair_targets = 15
        self.cls = BertMLMSBOHeads(config, self.bert.embeddings.word_embeddings.weight, max_pair_targets)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], BertForMLMSBOOutput]:
        r"""
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                Labels for computing the masked language modeling loss. Indices should be in `[-100, 0, ...,
                config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked),
        ...
        sequence_output, pooled_output = outputs[:2]
        # prediction_scores, seq_relationship_score = self.cls(sequence_output, pooled_output)
        prediction_scores, pair_target_scores = self.cls(sequence_output, pooled_output, pairs)

        total_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
            pair_target_logits = pair_target_scores.view(-1, pair_target_scores.size(-1))
            pair_targets = targets.view(-1)
            pair_loss = loss_fct(
                pair_target_logits,
                pair_targets,
                # size_average=False,
                # ignore_index= -100, # 默认就是-100
                # reduce=True
            )
            # print("masked_lm_loss",float(masked_lm_loss),"pairloss", float(pair_loss))
            total_loss = masked_lm_loss + pair_loss
            # total_loss = pair_loss

        return BertForMLMSBOOutput(
            loss=total_loss,
            prediction_logits=prediction_scores,
            seq_relationship_logits=pair_target_scores,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )
